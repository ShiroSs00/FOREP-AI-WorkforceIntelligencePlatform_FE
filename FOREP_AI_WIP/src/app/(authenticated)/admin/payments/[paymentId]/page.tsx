...
export default function AdminPaymentDetailPage() {
  ...
  const action = useMutation({ mutationFn: ({ type, note }: { type: "confirm" | "reject"; note?: string }) => type === "confirm" ? confirmAdminPayment(paymentId, { note }) : rejectAdminPayment(paymentId, { note }), onSuccess: () => { toast.success("Đã cập nhật thanh toán"); invalidateAdminLifecycleQueries(client); void client.invalidateQueries({ queryKey: queryKeys.adminPayment(paymentId) }); } });
  ...
  const canReview = hasPermission(user, "PAYMENT_CONFIRM") && (item?.status === "PENDING" || item?.status === "PROCESSING" || item?.status === "MANUAL_REVIEW");
