...
export default function PaymentResultPage() {
  ...
  const registration = useQuery({ queryKey: queryKeys.workspaceRegistration(registrationId), queryFn: () => getWorkspaceRegistration(registrationId ?? "", session.token ?? ""), enabled: !!registrationId && session.ready && !!session.token });
  ...
  const workspaceActive = Boolean(registrationItem?.workspaceId) && registrationItem?.registrationStatus === "ACTIVE";
  ...
  <div className="mt-6 grid gap-3 text-sm leading-6 text-muted-foreground">
    {item.status === "SUCCESS" ? <><p>Backend đã xác nhận thanh toán.</p>{workspaceActive ? <Link href="/login"><Button>Đăng nhập</Button></Link> : <p>Workspace đang được kích hoạt. Vui lòng kiểm tra lại sau.</p>}</> : null}
