...
export function AiHistoryView() {
  ...
  return <>
    ...
    <Card className="mb-5 grid gap-3 lg:grid-cols-5"><Field label="Chức năng" value={functionName} onChange={(event) => setFunctionName(event.target.value)} placeholder="Tên chức năng" /><Select label="Trạng thái" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">Tất cả</option><option value="SUCCESS">Thành công</option><option value="FAILED">Thất bại</option><option value="FALLBACK">Phân tích quy tắc</option><option value="PROCESSING">Đang xử lý</option><option value="CANCELLED">Đã hủy</option></Select><Field label="Người gọi" value={caller} onChange={(event) => setCaller(event.target.value)} placeholder="Tên người gọi" /><Field label="Từ ngày" type="datetime-local" value={from} onChange={(event) => setFrom(event.target.value)} /><Field label="Đến ngày" type="datetime-local" value={to} onChange={(event) => setTo(event.target.value)} /></Card>
