...
export function DepartmentManager({ readOnly = false }: { readOnly?: boolean }) {
  ...
  return <>
    <PageHeader eyebrow="DỮ LIỆU NHÂN SỰ" title="Phòng ban" description={readOnly ? "Danh mục phòng ban trong workspace. Chỉ HR có quyền thay đổi dữ liệu." : "Quản lý phòng ban dùng chung cho nhân viên, vị trí nghiệp vụ, task và AI mapping."} />
    ...
    {!query.isLoading && !query.error ? <div className={readOnly ? "" : "grid gap-5 xl:grid-cols-[1.35fr_0.85fr]"}>
      <div className="grid content-start gap-3">{rows.map((item) => <Card key={item.id}><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><h2 className="font-black">{item.name}</h2><StatusBadge value={item.status} /></div><p className="mt-1 text-xs font-bold tracking-[0.12em] text-muted-foreground">{item.code || "CHƯA CÓ MÃ"}</p><p className="mt-3 text-sm leading-6 text-muted-foreground">{item.description || "Chưa có mô tả."}</p><p className="mt-3 text-xs text-muted-foreground">Tạo {formatDateTime(item.createdAt)} · Cập nhật {formatDateTime(item.updatedAt)}</p></div>{!readOnly ? <div className="flex gap-2"><Button variant="secondary" onClick={() => startEdit(item)}><Pencil className="h-4 w-4" />Sửa</Button><Button variant={item.status === "ACTIVE" ? "outline" : "secondary"} disabled={lifecycle.isPending} onClick={() => { const active = item.status !== "ACTIVE"; if (!active && !window.confirm("Ngưng phòng ban có thể ảnh hưởng vị trí, nhân viên và task đang hoạt động. Tiếp tục?")) return; lifecycle.mutate({ id: item.id, active }); }}><Power className="h-4 w-4" />{item.status === "ACTIVE" ? "Ngưng" : "Kích hoạt"}</Button></div> : null}</div></Card>)}</div>
