...
import LanguageToggle from '../app/LanguageToggle.jsx'
...
function HeroSection() {
  ...
  const words = t('landing.headline', 'Reveal workforce risk before it becomes operational drag.').split(' ')
  ...
  return (
    <section ref={ref} className="relative isolate flex min-h-screen items-center overflow-hidden bg-[#020617] px-4 py-28 text-white sm:px-6 lg:px-8">
      ...
      <div className="absolute right-4 top-4 z-20 flex items-center gap-2 sm:right-6 lg:right-8">
        <LanguageToggle className="border-white/20 bg-white/10 text-white backdrop-blur hover:bg-white/15 hover:text-white dark:border-white/20 dark:bg-white/10 dark:text-white dark:hover:bg-white/15" />
      ...
      <div className="relative z-10 mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="max-w-5xl">
          ...
          <p className="hero-reveal mt-4 opacity-0 text-lg font-semibold text-slate-100">{t('landing.platform', 'AI Workforce Intelligence Platform')}</p>
          ...
          <p className="hero-reveal mt-6 inline-flex opacity-0 rounded-full border border-sky-400/25 bg-sky-400/10 px-4 py-2 text-sm font-semibold text-sky-100">
            {t('landing.trust', 'Not a surveillance tool. A transparency tool.')}
          </p>
          <p className="hero-reveal mt-6 max-w-3xl opacity-0 text-lg leading-8 text-slate-100">
            {t('landing.description', 'FOREP turns fragmented task, attendance, leave and performance signals into one intelligent workforce timeline so leaders can act before risk becomes escalation.')}
          </p>
          <div className="hero-reveal mt-10 flex flex-col gap-3 opacity-0 sm:flex-row">
            <Link to="/login" className="rounded-lg bg-[#0ea5e9] px-6 py-3 text-center text-sm font-semibold text-white shadow-sm transition hover:bg-sky-500">{t('landing.login', 'Login')}</Link>
            <a href="#features" className="rounded-lg border border-white/25 bg-white/15 px-6 py-3 text-center text-sm font-semibold text-white backdrop-blur transition hover:bg-white/20">{t('landing.explore', 'Explore Features')}</a>
          </div>
        </div>
          ...
          <div className="relative rounded-2xl border border-white/15 bg-white/[0.07] p-4 shadow-2xl shadow-sky-950/60 backdrop-blur-xl">
            <div className="rounded-xl border border-white/10 bg-[#07111f]/90 p-4">
              <div className="mb-5 flex items-center justify-between border-b border-white/10 pb-4">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.24em] text-sky-300">{t('landing.liveSignals', 'Live Workforce Signals')}</p>
                  <p className="mt-1 text-sm text-slate-300">{t('landing.signalDescription', 'Operational events flowing into FOREP')}</p>
                </div>
                <span className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-xs font-semibold text-emerald-200">{t('landing.aiReady', 'AI READY')}</span>
              </div>
